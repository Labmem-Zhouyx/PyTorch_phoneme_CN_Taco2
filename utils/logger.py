...
from .plot import plot_alignment_to_numpy, plot_reference_alignment_to_numpy, plot_spectrogram_to_numpy
...
class Tacotron2Logger(SummaryWriter):
    ...
    def log_training(self, loss, speaker_loss, ref_speaker_loss, kl_loss, grad_norm, learning_rate, duration, iteration):
            self.add_scalar("training.loss", loss, iteration)
            self.add_scalar("training.speaker_loss", speaker_loss, iteration)
            self.add_scalar("training.ReferenceSpeakerClassifer_loss", ref_speaker_loss, iteration)
            self.add_scalar("training.VAE_kl_loss", kl_loss, iteration)
    ...
    def log_validation(self, loss, speaker_loss, model, targets, predicts, iteration):
        ...
        _, mel_predicts, gate_predicts, alignments, ref_alignments, _, _, _, _ = predicts
        if len(targets) == 4:
        ...
        if ref_alignments != None:
            self.add_image(
                "reference_alignment",
                plot_reference_alignment_to_numpy(ref_alignments[idx].data.cpu().numpy().T),
                iteration, dataformats='HWC')
        self.add_image(
            "mel_target",
